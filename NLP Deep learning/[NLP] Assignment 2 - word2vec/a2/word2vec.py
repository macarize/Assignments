...
def naiveSoftmaxLossAndGradient(
    ...
    groundTruth = np.zeros((outsideVectors.shape[0], 1))
    groundTruth[outsideWordIdx] = 1

    probability = softmax(np.matmul(outsideVectors, centerWordVec)).reshape(-1, 1)

    loss = -np.log(probability[outsideWordIdx])

    gradCenterVec = np.matmul(outsideVectors.T, probability-groundTruth).reshape(-1, )
    gradOutsideVecs = np.matmul((probability-groundTruth), centerWordVec.reshape(1, -1))
...
def negSamplingLossAndGradient(
    ...
    loss = -np.log(sigmoid(outsideVectors[outsideWordIdx].T.dot(centerWordVec))) - np.sum(np.log(sigmoid(-outsideVectors[negSampleWordIndices].dot(centerWordVec))))

    gradCenterVec = (sigmoid(outsideVectors[outsideWordIdx].T.dot(centerWordVec))-1) * outsideVectors[outsideWordIdx] + \
    np.sum(np.expand_dims(1-sigmoid(-outsideVectors[negSampleWordIndices].dot(centerWordVec)), axis=1) * (outsideVectors[negSampleWordIndices]), axis=0)

    gradOutsideVecs = np.zeros_like(outsideVectors)
    gradOutsideVecs[outsideWordIdx] = (sigmoid(outsideVectors[outsideWordIdx].T.dot(centerWordVec))-1) * centerWordVec
    for i,neg_idx in enumerate(negSampleWordIndices):
        gradOutsideVecs[neg_idx] += (1 - sigmoid(-outsideVectors[negSampleWordIndices].dot(centerWordVec))[i]) * centerWordVec
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    grad = np.zeros_like(centerWordVec)

    for i in range(len(outsideWords)):
        outsideWordInd = word2Ind[outsideWords[i]]
        word_loss, gradCenterVecs_i, gradOutsideVectors_i = word2vecLossAndGradient(centerWordVec, outsideWordInd, outsideVectors, dataset)
        loss += word_loss
        grad += gradCenterVecs_i
        gradOutsideVectors += gradOutsideVectors_i

    gradCenterVecs[centerWordInd, :] = grad
    ### END YOUR CODE
